...
public class 绩效FIVE {
    ...
    private static void getSql(StringBuilder sql, List<AccMangeRel> accMangeRels) {
        ...
        for (AccMangeRel accMangeRel : accMangeRels){
            ...
            sql.append("'770088',");
            sql.append("(select cif_no_new from icore_ecif_bth.cif_info_mid where cif_no_old = '"+accMangeRel.get客户号()+"'),");
            sql.append("(select acc_no_new from ac_seqn_map where acc_no_old = '"+accMangeRel.get电子账号()+"' and acc_seqn_old = '"+accMangeRel.get账户序号()+"'),");
            sql.append("(select acc_id from mdm_acc_rel where cert_no in (select bnd_cert_no from acc_bind_ban where bnd_phone_no = '"+accMangeRel.get客户经理客户号()+"') and acc_name='"+accMangeRel.get客户经理名()+"' and opn_br_no='770088'),");
            ...
            sql.append("(select cif_no from mdm_acc_rel where cert_no in (select bnd_cert_no from acc_bind_ban where bnd_phone_no = '"+accMangeRel.get客户经理客户号()+"') and acc_name='"+accMangeRel.get客户经理名()+"' and opn_br_no='770088'),");
            sql.append("null,");
