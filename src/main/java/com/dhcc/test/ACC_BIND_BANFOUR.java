...
public class ACC_BIND_BANFOUR {
    ...
    public static void getSql(StringBuilder sql, List<AccBindBan> accBindBans){
        ...
        for (AccBindBan accBindBan : accBindBans){
            ...
            sql.append("(select ACC_ID from MDM_ACC_REL where CERT_NO='"+accBindBan.getBnd_cert_no()+"' and OPN_BR_NO='770088'),");
            sql.append("1,");
            sql.append("(select ACC_NAME from MDM_ACC_REL where CERT_NO='"+accBindBan.getBnd_cert_no()+"' and OPN_BR_NO='770088'),");
            sql.append("'AT00',");
        ...
        // values((select distinct(ACC_NO_NEW) from AC_SEQN_MAP where ACC_NO_OLD = ''),null,1,'name','AT00','bnd_acc_no','name','20180401','ZJ01','bnd_cert_no','bnd_phone_no','BF00','AF00','BS00','770088');
